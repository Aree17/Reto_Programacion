...
// --------- Agregar Objeto (solo recibe idCajon) ----------
function ObjetoEntryForm({ idCajon, onObjetoCreated }: ObjetoEntryFormProps) {
  const nombre = useSignal('');
  ...
  const createObjeto = async () => {
    try {
      if (nombre.value.trim() && tipo.value.trim() && tamanio.value.trim()) {
        await ObjetoService.create(nombre.value, tipo.value, tamanio.value, idCajon);
        if (onObjetoCreated) onObjetoCreated();
        nombre.value = '';
        tipo.value = '';
        tamanio.value = '';
        dialogOpened.value = false;
        Notification.show('Objeto creado', { duration: 3000, position: 'bottom-end', theme: 'success' });
      } else {
        Notification.show('No se pudo crear, faltan datos', { duration: 3000, position: 'top-center', theme: 'error' });
      }
    } catch (error) {
    ...
    <>
      <Dialog
        ...
        footer={
          <>
            <Button onClick={() => (dialogOpened.value = false)}>Cancelar</Button>
            <Button onClick={createObjeto} theme="primary">Crear</Button>
          </>
        }
      >
        <VerticalLayout style={{ alignItems: 'stretch', width: '18rem', maxWidth: '100%' }}>
          ...
          <TextField
            label="Tipo"
            placeholder="Ingrese el tipo (ej. ROPA, PAPELERIA, CABLES, OTRO)"
            value={tipo.value}
            onValueChanged={(evt) => (tipo.value = evt.detail.value)}
          />
          <TextField
            label="Tamaño"
            placeholder="Ingrese el tamaño (ej. PEQUENO, MEDIANO, GRANDE)"
            value={tamanio.value}
            onValueChanged={(evt) => (tamanio.value = evt.detail.value)}
          />
        </VerticalLayout>
      </Dialog>
      <Button onClick={() => (dialogOpened.value = true)}>Agregar Objeto</Button>
    </>
  );
}
